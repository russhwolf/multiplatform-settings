...
public class QdbmRelicSettings(private val path: String) : Settings {
    ...
    private fun saveBytes(key: String, bytes: ByteArray): Unit = dbmOperation { dbm ->
        dbm_store(dbm, datumOf(key), datumOf(bytes), DBM_REPLACE.toInt())
    }

    private fun loadBytes(key: String): ByteArray? = dbmOperation { dbm ->
    ...
    private fun ByteArray.toLong(): Long = foldIndexed(0) { index, total: Long, byte: Byte ->
        ((0xff.toLong() and byte.toLong()) shl index * Byte.SIZE_BITS) or total
    }

    private fun ByteArray.toInt(): Int = foldIndexed(0) { index, total: Int, byte: Byte ->
        ((0xff and byte.toInt()) shl index * Byte.SIZE_BITS) or total
    }

    private fun Long.toByteArray(): ByteArray = ByteArray(Long.SIZE_BYTES) { index ->
        ((this shr (Byte.SIZE_BITS * index)) and 0xff).toByte()
    }

    private fun Int.toByteArray(): ByteArray = ByteArray(Int.SIZE_BYTES) { index ->
        ((this shr (Byte.SIZE_BITS * index)) and 0xff).toByte()
    }

    private fun CValue<datum>.toKString(): String? = toByteArray()?.decodeToString()
    private fun CValue<datum>.toByteArray(): ByteArray? = useContents {
    ...
    private fun MemScope.datumOf(string: String): CValue<datum> = datumOf(string.encodeToByteArray())
    private fun MemScope.datumOf(bytes: ByteArray): CValue<datum> = cValue {
